...
import HttpStatusBulding from './controllers/bulding.controller.js';
...
import routesBulding from './routes/bulding.routes.js';
...
//Bulding
app.use('/', routesBulding);
app.get('/', (req, res) => res.send(new Response(HttpStatusBulding.OK.code, HttpStatusBulding.OK.status, 'Airlux DB distante API, v1.0.0 - All Systems Go')));
app.all('*', (req, res) => res.status(HttpStatusBulding.NOT_FOUND.code)
  .send(new Response(HttpStatusBulding.NOT_FOUND.code, HttpStatusBulding.NOT_FOUND.status, 'Route does not exist on the server')));
