...
export const createRoom = async (req:Request, res:Response) => {
  ...
  try {
    ...
    var results:Array<any> = await processData(QUERY.SELECT_FLOOR, req.body.floor_id);
    if (results.length === 0) {
...
export const getRooms = async (req:Request, res:Response) => {
  ...
  try {
    var results:Array<any> = await processDatas(QUERY.SELECT_ROOMS);
    if (results.length === 0) {
...
export const getRoom = async(req:Request, res:Response) => {
  ...
  try {
    var results:Array<any> = await processData(QUERY.SELECT_ROOM, req.params.id);
    if (results.length === 0) {
      res.status(HttpStatus.NOT_FOUND.code)
...
export const deleteRoom = async (req:Request, res:Response) => {
  ...
  try {
    var results = await processData(QUERY.SELECT_ROOM, req.params.id);
    if (results.length === 0) {
      return res.status(HttpStatus.NOT_FOUND.code)
        .send(new ResponseFormat(HttpStatus.NOT_FOUND.code, HttpStatus.NOT_FOUND.status, `Room by id ${req.params.id} was not found`));
    } 
